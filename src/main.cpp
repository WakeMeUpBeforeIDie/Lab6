...
int main(int argc, char* argv[]) {
    ...
    if (generator.saveToFile(outputFile, generatedText)) {
        std::cout << "Текст успешно сгенерирован и сохранен в " << outputFile << std::endl;
        std::cout << "\nСгенерированный текст (первые 500 символов):" << std::endl;
        std::cout << generatedText.substr(0, std::min(size_t(500), generatedText.size())) << "..." << std::endl;
    } else {
